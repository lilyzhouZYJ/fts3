...
std::map<Scheduler::VoName, std::list<TransferFile>> DeficitScheduler::doSchedule(
    std::map<Pair, int> &slotsPerLink, 
    std::vector<QueueId> &queues
){
    FTS3_COMMON_LOGGER_NEWLOG(INFO) << "DeficitScheduler: in doSchedule (lzhou)" << commit;
...
std::map<Pair, std::map<Scheduler::VoName, double>> DeficitScheduler::getVoWeightsInEachPair(
    std::map<Pair, int> &slotsPerLink, 
    std::vector<QueueId> &queues,
    std::vector<QueueId> &unschedulable
) {
...
std::map<Scheduler::VoName, std::map<Scheduler::ActivityName, double>> DeficitScheduler::getActivityWeights(std::vector<QueueId> &queues)
{
...
void DeficitScheduler::getTransferFilesBasedOnSlots(
    Pair pair,
...
std::map<Scheduler::VoName, std::map<Scheduler::ActivityName, int>> DeficitScheduler::assignSlotsUsingDeficitPriorityQueue(
    int maxSlots,
    ...
    for (auto j = this->allQueueDeficitSlots.begin(); j != this->allQueueDeficitSlots.end(); j++) {
        VoName voName = j->first;
    ...
    for (int i = 0; i < availableSlots; i++) {
        ...
        deficit -= 1;
        this->allQueueDeficitSlots[voName][activityName] = deficit;
        ...
        if (assignedSlotCounts[voName][activityName] < queueSubmittedCounts[voName][activityName]) {
            ...
            this->allQueueDeficitSlots[voName][activityName] = 0;
        }
    }
...
std::map<Scheduler::VoName, std::map<Scheduler::ActivityName, long long>> DeficitScheduler::computeActiveCounts(
    std::string src,
    std::string dest,
    std::map<VoName, std::map<ActivityName, double>> &voActivityWeights
){
...
std::map<Scheduler::VoName, std::map<Scheduler::ActivityName, long long>> DeficitScheduler::computeSubmittedCounts(
    std::string src,
    std::string dest,
    std::map<VoName, std::map<ActivityName, double>> &voActivityWeights
){
...
std::map<Scheduler::VoName, std::map<Scheduler::ActivityName, int>> DeficitScheduler::computeShouldBeSlots(
    int maxPairSlots,
    std::map<VoName, double> &voWeights,
    std::map<VoName, std::map<ActivityName, double>> &voActivityWeights,
    std::map<VoName, std::map<ActivityName, long long>> &queueActiveCounts,
...
std::map<Scheduler::VoName, int> DeficitScheduler::assignShouldBeSlotsToVos(
    std::map<VoName, double> &voWeights,
    int maxPairSlots,
    std::map<VoName, std::map<ActivityName, long long>> &queueActiveCounts,
    std::map<VoName, std::map<ActivityName, long long>> &queueSubmittedCounts
...
std::map<Scheduler::ActivityName, int> DeficitScheduler::assignShouldBeSlotsToActivities(
...
std::map<std::string, int> DeficitScheduler::assignShouldBeSlotsUsingHuntingtonHill(
...
void DeficitScheduler::computeDeficitSlots(
    ...
    for (auto i = queueActiveCounts.begin(); i != queueActiveCounts.end(); i++) {
        ...
        for (auto j = activityActiveCount.begin(); j != activityActiveCount.end(); j++) {
            ...
            if (activeCount + queueSubmittedCounts[voName][activityName] == 0) {
                ...
                if (this->allQueueDeficitSlots.find(voName) == this->allQueueDeficitSlots.end() || this->allQueueDeficitSlots[voName].find(activityName) == this->allQueueDeficitSlots[voName].end()) {
                    // Initialize
                    this->allQueueDeficitSlots[voName][activityName] = 0;
                }
                this->allQueueDeficitSlots[voName][activityName] += (long long) shouldBeAllocatedCount - activeCount;

                FTS3_COMMON_LOGGER_NEWLOG(INFO) << "deficit[vo=" << voName << "]"
                                                << "[activity=" << activityName << "]"
                                                << " = " << this->allQueueDeficitSlots[voName][activityName] << " "
                                                << "(lzhou)" << commit;
            }
        }
    }
